...
export class StartAnimationComponent implements OnInit, AfterViewInit {
  ...
  resizeEvent(){  this.initCanvas()
  }
  @HostListener("document:mousemove", ['$event'])
  mousemoveEvent($event: Event){  console.log($event); this.moveMousePoint($event)
  }
  @HostListener("document:mouseleave", ['$event'])
  mouseleaveEvent(){  this.removeMousePoint()
  }
...
getLines(point : Point, index: number, isMouseLine: boolean) {
  for (let i = 0; i < this.points.length; i++) {
    if (point.pointsAreClose(this.points[i], 0.2 * this.c.width) && !this.points[i].isMousePoint && i > index) {
...
containsLine(line: Line) {
  return this.linesToDraw.find((l) => l.x1 === line.x2 && l.y1 === line.y2 && l.x2 === line.x1 && l.y2 === line.y1);
}
...
drawPoints() {
  this.points.forEach((p) => {
    this.ctx.fillStyle = p.isMousePoint ? 'hsl(270, 80%, 50%)' : 'hsl(180, 100%, 15%)';
    this.ctx.beginPath();
...
drawLines() {
  this.linesToDraw.forEach((l) => {
    this.ctx.strokeStyle = l.isMouseLine? this.getLineGradient(l) : this.getLineColor(l);
    this.ctx.beginPath();
